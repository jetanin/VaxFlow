  ...
  return (
    <div className="app">
      <div className="topbar">
        <div>
          <h1 className="title">
            💉 VacFlow — Predictive Vaccine Shared Inventory
          </h1>
          <p className="subtitle">
            เครือข่ายแบ่งปันวัคซีน · ลด Vaccine Wastage
          </p>
        </div>
        <div className="user-box">
          <span className="muted">
            {isAdmin ? "🛡️ " : isDirector ? "🧑‍⚕️ " : "🏥 "}
            {me?.name || me?.hospital_id}
          </span>
          <span
            className={`badge ${isAdmin ? "red" : isDirector ? "yellow" : "green"}`}
          >
            {isAdmin ? "ADMIN" : isDirector ? "ผอ.รพ." : "HOSPITAL"}
          </span>
          <button
            className="tab"
            onClick={async () => {
      ...
      <div className="kpi-row">
        <div className="kpi">
          <div className="label">โรงพยาบาลที่เชื่อมต่อ</div>
          <div className="value">{summary.hospitals}</div>
        </div>
        <div className="kpi">
          <div className="label">🔴 วิกฤต (≤14 วัน)</div>
          <div className="value">{summary.red_items}</div>
        </div>
        <div className="kpi">
          <div className="label">🟡 ใกล้หมดอายุ (≤21 วัน)</div>
          <div className="value">{summary.yellow_items}</div>
        </div>
        <div className="kpi">
          <div className="label">💉 ขวดที่เปิดแล้ว</div>
          <div className="value">{summary.opened_vials}</div>
        </div>
        <div className="kpi">
          <div className="label">โดสคงคลังรวม</div>
          <div className="value">
            {Number(summary.total_doses).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <p className="subtitle" style={{ marginTop: 16 }}>
        VacFlow · Logistics Innovation Hackathon 2026
      </p>
    </div>
  );
}
