...
export default function Borrow() {
  ...
  useEffect(() => {
    if (drug) api.lenders(drug).then((rows) => {
      setLenders(rows);
  ...
  async function submit(e) {
    ...
    setMsg(null);
    try {
      await api.createBorrow({ to_hospital: toHospital, drug, quantity: parseFloat(qty), reason });
  ...
  async function act(id, status) {
    try { await api.actBorrow(id, status); refresh(); }
    catch (err) { setMsg("⚠️ " + err.message); }
  }
  ...
  return (
    <div className="row">
      ...
      <div className="panel">
        ...
        {redDrugs.length === 0 ? (
          <p className="muted">โรงพยาบาลของคุณไม่มียาสถานะ 🔴 ขาดแคลน — ยังไม่ต้องยืม 🎉</p>
        ) : (
          <form onSubmit={submit}>
            <label className="muted">ยาที่ขาดแคลน (🔴 เท่านั้น)</label>
            <select value={drug} onChange={(e) => setDrug(e.target.value)}>
              {redDrugs.map((d) => (
                <option key={d.drug} value={d.drug}>{d.drug} — {d.desc_th}</option>
              ))}
            </select>
            ...
            <label className="muted">เหตุผล (ไม่บังคับ)</label>
            <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="เช่น คนไข้เพิ่มฉับพลัน" />

            <button className="tab active" type="submit" disabled={!toHospital}>ส่งคำขอยืมยา</button>
          </form>
        )}
      ...
      <div className="panel">
        ...
        <table>
          ...
          <tbody>
              ...
              <tr key={r.id}>
                ...
                <td>
                  {r.direction === "incoming" && r.status === "pending" && (
                    <>
                      <button className="mini ok" onClick={() => act(r.id, "approved")}>อนุมัติ</button>
            ...
            {requests.length === 0 && (
              <tr><td colSpan="6" className="muted">ยังไม่มีคำขอ</td></tr>
            )}
          </tbody>
        </table>
      </div>
