...
- **Adminer (จัดการ DB):** http://localhost:8081 — System `PostgreSQL` · Server `db` · user/pass/db = `vacflow`
- **Postgres:** localhost:5432 (user/pass/db = `vacflow`)
...
| --- | --- | --- | --- |
| `admin` | 🛡️ admin | เห็นพยากรณ์/คลังยา **ทุกโรงพยาบาล** | `vacflow123` |
| `HOSP_001` … `HOSP_013` | 🏥 hospital | เห็นเฉพาะ **ของตัวเอง** + ยืมยาได้ | `vacflow123` |
...
python scripts/fl_client.py --hospital HOSP_001 --server http://localhost:4000 --token vacflow-reseed
python scripts/fl_client.py --hospital HOSP_002 --server http://localhost:4000 --token vacflow-reseed
#   ... (ทุก รพ.) ...

# (ศูนย์กลาง) รวม weight ทุก รพ. ด้วย FedAvg -> เขียน weight กลางใหม่ลงตาราง weights
curl -X POST http://localhost:4000/api/fl/aggregate -H "X-FL-Token: vacflow-reseed" `
