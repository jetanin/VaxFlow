...
│  UI วัคซีน   │         │ auth/seed/audit│        │ (vaxflow)  │
└──────────────┘         └────────────────┘        └────────────┘
                                                        │
┌────────────────────────────────────────────┐         │
...
# 4) login: admin / vaxflow123  หรือ  HOSP_001 / vaxflow123
```
...
VaxFlow ต่อ HIS เดิม (เช่น **HOSxP**) แบบ **อ่านอย่างเดียว** ผ่าน user `vaxflow_ro` ที่มีสิทธิ์
`SELECT` เฉพาะ **view ที่ตัด PII แล้ว** (`vw_vaxflow_*`) — บังคับ Data Minimization ที่ระดับ DB
...
│   └ vw_vaxflow_* (aggregate)     ✓ เข้าได้    │
│              │ vaxflow_ro (SELECT view เท่านั้น)│
│              ▼                                │
│   VaxFlow Edge Agent (FastAPI)                │
...
# Adminer: http://localhost:8081  (login เป็น vaxflow_ro แล้วลองยิง PII ดูว่าโดนบล็อก)
# พิสูจน์สิทธิ์: mysql -h 127.0.0.1 -u vaxflow_ro -p mock_hosxp < mock-his/tests/test_ro_cannot_read_pii.sql
```

> รายละเอียด Tier 1/2/3 + adapter pattern ดูใน Integration Plan

> ⚠️ เปลี่ยน credential ฐานข้อมูล (medcast → vaxflow) แล้ว — ถ้าเคยรันมาก่อนต้องล้าง volume เดิม:
...
- Login เริ่มต้น: `admin` / `HOSP_001…` รหัส `vaxflow123`
...
├── docker-compose.mock.yml         # mock-his + vaxflow-edge + adminer
