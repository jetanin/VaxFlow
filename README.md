...
- **Federated Learning:** `fedavg_4hosp` ให้ความแม่น **ใกล้เคียง/เท่ากับ centralized (รวมข้อมูลไว้ที่เดียว)** ทั้งที่ข้อมูลดิบไม่เคยออกจากโรงพยาบาล ✅
...
| ------------ | --------------------------------- | --------------------------------------------- |
| **frontend** | React + Vite + Leaflet + Recharts | 3 พาเนล (Map / AI / Privacy) เหมือน dashboard |
| **backend**  | Node.js + Express + pg            | REST API + seed ข้อมูลลง Postgres             |
| **database** | PostgreSQL 16                     | เก็บ hospitals / forecasts / weights          |
...
- Frontend: http://localhost:8080 · API: http://localhost:4000/api/health

> รายละเอียด endpoint + วิธี dev แบบ local (npm) ดูที่ [webapp/README.md](webapp/README.md)
...
### 4. (ตัวเลือก) สร้างข้อมูล 4 โรงพยาบาล + export weight

```powershell
python scripts/generate_hospital_data.py   # -> data/hospitals/HOSP_00X.csv
