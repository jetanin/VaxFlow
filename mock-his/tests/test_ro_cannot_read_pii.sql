-- Regression guard — รันในฐานะ vaxflow_ro:
--   mysql -h 127.0.0.1 -u vaxflow_ro -p mock_hosxp < test_ro_cannot_read_pii.sql
USE mock_hosxp;

-- ✅ ต้องได้ผล (view ที่ตัด PII แล้ว)
SELECT * FROM vw_vaxflow_drug_usage  LIMIT 3;
SELECT * FROM vw_vaxflow_inventory   LIMIT 3;
SELECT * FROM vw_vaxflow_drug_master LIMIT 3;
